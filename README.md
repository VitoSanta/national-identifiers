...
| `vat` | Value-added tax registration number | 31 countries |
...
The current implementation covers 47 jurisdictions: Italy has full name,
date, gender and place consistency, while 46 additional formats expose
partial checks for encoded birth date, gender or administrative place code.
Ten decode a **national identity document** validated independently of the
country's tax identifier — Egypt (National ID), France (NIR), Kuwait (Civil
ID), Mexico (CURP), Vietnam (CCCD), the UAE (Emirates ID), Bahrain (CPR),
Qatar (QID), Bangladesh (17-digit NID) and Taiwan (National ID). Taiwan is a
jurisdiction outside the 195-state tax-id set, supported for identity
consistency only. See the [capability matrix](docs/IDENTITY-CONSISTENCY.md)
and [expansion research](docs/IDENTITY-CONSISTENCY-RESEARCH.md). A `match` is
not identity verification, and no personal value is logged or echoed back.
